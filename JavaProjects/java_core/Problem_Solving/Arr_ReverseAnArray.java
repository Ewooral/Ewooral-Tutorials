...
public ArrayList<Integer>reverseArray(Integer[]arr){for(int i=arr.length-1;i!=-1;i--){int lastElem=arr[i];reversed.add(lastElem);}return reversed;}

// using Collection.reverse()
public void reverse(Integer a[]){Collections.reverse(Arrays.asList(a));System.out.println(Arrays.asList(a));
